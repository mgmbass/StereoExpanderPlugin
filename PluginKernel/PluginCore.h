...
public:
	...
	double att3_cooked = 1.0;
	double LF_cooked = 1.0;
...
private:
	...
	int APF_On = 0;
	enum class APF_OnEnum { SWITCH_OFF,SWITCH_ON };	// to compare: if(compareEnumToInt(APF_OnEnum::SWITCH_OFF, APF_On)) etc... 

	int eq_Listen = 0;
	enum class eq_ListenEnum { SWITCH_OFF,SWITCH_ON };	// to compare: if(compareEnumToInt(eq_ListenEnum::SWITCH_OFF, eq_Listen)) etc... 
